...
export function IntakeBasicInfoStep({
  form,
  setForm,
}: IntakeBasicInfoStepProps) {
  return (
    <IntakeSectionCard
      icon={
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="flex gap-3">
            {GENDERS.map(({ value, label }) => (
              <Button
                ...
                className={cn(
                  "h-auto flex-1 py-3 rounded-xl border-0 text-sm font-semibold transition-all duration-150 active:scale-[0.98]",
                  form.gender === value
                    ? "bg-primary text-primary-foreground"
                    : "bg-white text-muted-foreground hover:bg-cs-surface-container-highest",
                )}
              >
                {label}
              </Button>
        ...
        <div className="md:col-span-2">
          ...
          <input
            ...
            className="w-full max-w-full h-11 px-4 rounded-xl border border-border bg-white text-sm text-cs-on-surface focus:outline-none focus:ring-2 focus:ring-primary/30 transition"
          />
        </div>
      </div>
    </IntakeSectionCard>
